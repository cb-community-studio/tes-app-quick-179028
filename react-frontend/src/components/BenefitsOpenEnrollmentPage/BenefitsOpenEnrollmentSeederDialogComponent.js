...
import seederData from "./benefitsOpenEnrollment.seeder.json";
...
const BenefitsOpenEnrollmentSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("benefitsOpenEnrollment").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(BenefitsOpenEnrollmentSeederDialogComponent);
