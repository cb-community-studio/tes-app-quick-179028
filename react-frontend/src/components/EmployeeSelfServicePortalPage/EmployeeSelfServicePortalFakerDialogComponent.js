...
import employeeSelfServicePortalFakerFactory from "./employeeSelfServicePortalFakerFactory";
...
const EmployeeSelfServicePortalFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = employeeSelfServicePortalFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("employeeSelfServicePortal").create(elem));
...
export default connect(null, mapDispatch)(EmployeeSelfServicePortalFakerDialogComponent);
