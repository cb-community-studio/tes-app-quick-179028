...
import seederData from "./employeeSelfServicePortal.seeder.json";
...
const EmployeeSelfServicePortalSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("employeeSelfServicePortal").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(EmployeeSelfServicePortalSeederDialogComponent);
