...
import workforceDiversityAndInclusiFakerFactory from "./workforceDiversityAndInclusiFakerFactory";
...
const WorkforceDiversityAndInclusiFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = workforceDiversityAndInclusiFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("workforceDiversityAndInclusi").create(elem));
...
export default connect(null, mapDispatch)(WorkforceDiversityAndInclusiFakerDialogComponent);
