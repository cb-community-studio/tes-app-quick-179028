...
import seederData from "./workforceDiversityAndInclusi.seeder.json";
...
const WorkforceDiversityAndInclusiSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("workforceDiversityAndInclusi").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(WorkforceDiversityAndInclusiSeederDialogComponent);
