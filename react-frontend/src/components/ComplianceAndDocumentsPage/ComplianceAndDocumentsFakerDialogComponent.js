...
import complianceAndDocumentsFakerFactory from "./complianceAndDocumentsFakerFactory";
...
const ComplianceAndDocumentsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = complianceAndDocumentsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("complianceAndDocuments").create(elem));
...
export default connect(null, mapDispatch)(ComplianceAndDocumentsFakerDialogComponent);
