...
import seederData from "./complianceAndDocuments.seeder.json";
...
const ComplianceAndDocumentsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("complianceAndDocuments").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(ComplianceAndDocumentsSeederDialogComponent);
