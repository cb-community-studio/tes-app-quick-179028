...
import performanceReviewFakerFactory from "./performanceReviewFakerFactory";
...
const PerformanceReviewFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = performanceReviewFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("performanceReview").create(elem));
...
export default connect(null, mapDispatch)(PerformanceReviewFakerDialogComponent);
