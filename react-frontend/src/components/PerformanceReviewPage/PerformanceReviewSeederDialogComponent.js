...
import seederData from "./performanceReview.seeder.json";
...
const PerformanceReviewSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("performanceReview").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(PerformanceReviewSeederDialogComponent);
