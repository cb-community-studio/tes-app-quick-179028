...
import seederData from "./benefitsAdministration.seeder.json";
...
const BenefitsAdministrationSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("benefitsAdministration").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(BenefitsAdministrationSeederDialogComponent);
