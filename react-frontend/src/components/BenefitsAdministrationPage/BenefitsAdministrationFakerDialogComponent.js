...
import benefitsAdministrationFakerFactory from "./benefitsAdministrationFakerFactory";
...
const BenefitsAdministrationFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = benefitsAdministrationFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("benefitsAdministration").create(elem));
...
export default connect(null, mapDispatch)(BenefitsAdministrationFakerDialogComponent);
