...
import successionPlanningFakerFactory from "./successionPlanningFakerFactory";
...
const SuccessionPlanningFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = successionPlanningFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("successionPlanning").create(elem));
...
export default connect(null, mapDispatch)(SuccessionPlanningFakerDialogComponent);
