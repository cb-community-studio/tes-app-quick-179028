...
import seederData from "./successionPlanning.seeder.json";
...
const SuccessionPlanningSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("successionPlanning").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(SuccessionPlanningSeederDialogComponent);
