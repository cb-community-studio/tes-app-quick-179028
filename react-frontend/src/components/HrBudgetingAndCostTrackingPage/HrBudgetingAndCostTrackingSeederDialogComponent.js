...
import seederData from "./hrBudgetingAndCostTracking.seeder.json";
...
const HrBudgetingAndCostTrackingSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("hrBudgetingAndCostTracking").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(HrBudgetingAndCostTrackingSeederDialogComponent);
