...
import hrBudgetingAndCostTrackingFakerFactory from "./hrBudgetingAndCostTrackingFakerFactory";
...
const HrBudgetingAndCostTrackingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = hrBudgetingAndCostTrackingFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("hrBudgetingAndCostTracking").create(elem));
...
export default connect(null, mapDispatch)(HrBudgetingAndCostTrackingFakerDialogComponent);
