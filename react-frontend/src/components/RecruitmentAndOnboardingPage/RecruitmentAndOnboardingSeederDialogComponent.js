...
import seederData from "./recruitmentAndOnboarding.seeder.json";
...
const RecruitmentAndOnboardingSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("recruitmentAndOnboarding").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(RecruitmentAndOnboardingSeederDialogComponent);
