...
import recruitmentAndOnboardingFakerFactory from "./recruitmentAndOnboardingFakerFactory";
...
const RecruitmentAndOnboardingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = recruitmentAndOnboardingFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("recruitmentAndOnboarding").create(elem));
...
export default connect(null, mapDispatch)(RecruitmentAndOnboardingFakerDialogComponent);
