...
import employeeRelationsFakerFactory from "./employeeRelationsFakerFactory";
...
const EmployeeRelationsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = employeeRelationsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("employeeRelations").create(elem));
...
export default connect(null, mapDispatch)(EmployeeRelationsFakerDialogComponent);
