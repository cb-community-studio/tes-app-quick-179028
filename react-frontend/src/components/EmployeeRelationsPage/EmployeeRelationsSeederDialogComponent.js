...
import seederData from "./employeeRelations.seeder.json";
...
const EmployeeRelationsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("employeeRelations").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(EmployeeRelationsSeederDialogComponent);
