...
import seederData from "./timeTracking.seeder.json";
...
const TimeTrackingSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("timeTracking").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(TimeTrackingSeederDialogComponent);
