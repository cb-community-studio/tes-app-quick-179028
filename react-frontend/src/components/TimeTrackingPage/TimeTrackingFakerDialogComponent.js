...
import timeTrackingFakerFactory from "./timeTrackingFakerFactory";
...
const TimeTrackingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = timeTrackingFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("timeTracking").create(elem));
...
export default connect(null, mapDispatch)(TimeTrackingFakerDialogComponent);
