...
import attendanceAndLeaveManagementFakerFactory from "./attendanceAndLeaveManagementFakerFactory";
...
const AttendanceAndLeaveManagementFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = attendanceAndLeaveManagementFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("attendanceAndLeaveManagement").create(elem));
...
export default connect(null, mapDispatch)(AttendanceAndLeaveManagementFakerDialogComponent);
