...
import seederData from "./attendanceAndLeaveManagement.seeder.json";
...
const AttendanceAndLeaveManagementSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("attendanceAndLeaveManagement").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(AttendanceAndLeaveManagementSeederDialogComponent);
