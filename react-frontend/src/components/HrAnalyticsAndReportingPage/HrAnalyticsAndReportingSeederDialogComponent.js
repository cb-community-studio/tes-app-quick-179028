...
import seederData from "./hrAnalyticsAndReporting.seeder.json";
...
const HrAnalyticsAndReportingSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("hrAnalyticsAndReporting").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(HrAnalyticsAndReportingSeederDialogComponent);
