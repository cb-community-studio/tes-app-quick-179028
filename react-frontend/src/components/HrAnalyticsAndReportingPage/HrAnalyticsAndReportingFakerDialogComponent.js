...
import hrAnalyticsAndReportingFakerFactory from "./hrAnalyticsAndReportingFakerFactory";
...
const HrAnalyticsAndReportingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = hrAnalyticsAndReportingFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("hrAnalyticsAndReporting").create(elem));
...
export default connect(null, mapDispatch)(HrAnalyticsAndReportingFakerDialogComponent);
