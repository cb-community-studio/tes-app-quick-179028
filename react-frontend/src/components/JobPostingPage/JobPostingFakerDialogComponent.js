...
import jobPostingFakerFactory from "./jobPostingFakerFactory";
...
const JobPostingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = jobPostingFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("jobPosting").create(elem));
...
export default connect(null, mapDispatch)(JobPostingFakerDialogComponent);
