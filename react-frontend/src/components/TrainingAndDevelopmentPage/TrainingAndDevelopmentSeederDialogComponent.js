...
import seederData from "./trainingAndDevelopment.seeder.json";
...
const TrainingAndDevelopmentSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("trainingAndDevelopment").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(TrainingAndDevelopmentSeederDialogComponent);
