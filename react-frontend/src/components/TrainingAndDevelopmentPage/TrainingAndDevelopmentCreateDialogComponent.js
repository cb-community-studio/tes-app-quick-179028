...
const TrainingAndDevelopmentCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        ...
        const result = await client.service("trainingAndDevelopment").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info trainingAndDevelopment created successfully" });
    ...
    return (
        <Dialog header="Create" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div role="trainingAndDevelopment-create-dialog-component">
            <div>
