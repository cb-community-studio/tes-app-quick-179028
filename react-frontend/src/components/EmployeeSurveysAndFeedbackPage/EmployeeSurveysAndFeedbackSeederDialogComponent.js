...
import seederData from "./employeeSurveysAndFeedback.seeder.json";
...
const EmployeeSurveysAndFeedbackSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("employeeSurveysAndFeedback").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(EmployeeSurveysAndFeedbackSeederDialogComponent);
