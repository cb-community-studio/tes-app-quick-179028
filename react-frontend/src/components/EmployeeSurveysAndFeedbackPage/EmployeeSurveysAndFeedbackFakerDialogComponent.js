...
import employeeSurveysAndFeedbackFakerFactory from "./employeeSurveysAndFeedbackFakerFactory";
...
const EmployeeSurveysAndFeedbackFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = employeeSurveysAndFeedbackFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("employeeSurveysAndFeedback").create(elem));
...
export default connect(null, mapDispatch)(EmployeeSurveysAndFeedbackFakerDialogComponent);
